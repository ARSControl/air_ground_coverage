...
# @numba.njit(parallel=True, cache=True)
# def compute_voronoi_partitioning(xy_grid, robot_positions, theta, robot_range, gamma=10.0):
#     n_points = xy_grid.shape[0]
#     n_robots = robot_positions.shape[0]
...
def agent_fov(state, robot_range, fov_angle):
    """
    ...
    """
    x, y, theta = state
    th_arc = np.linspace(-fov_angle/2, fov_angle/2, num=10) + theta
