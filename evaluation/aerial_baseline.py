...
if __name__ == "__main__":
    ...
    for ep in range(params.num_episodes):
        ...
        for step in range(params.num_steps):
            ...
            if EVAL:
                kl_divergence[ep, step] = eval_kl_divergence(ground_target_pdf, aerial_gp_mean)
                # if step % print_freq == 0 and verbose:
                #     print(f"Step {step + 1} - KL Divergence: {kl_divergence[ep, step]:.4f}")
        ...
        if EVAL:
            w_dist = eval_wasserstein(ground_target_pdf, aerial_gp_mean)
    ...
    if EVAL:
        ...
        np.save(
            os.path.join(results_path, "aerial_kl_divergence2.npy"),
            kl_divergence
        )
        np.save(
            os.path.join(results_path, "aerial_wasserstein_distance2.npy"),
            wasserstein_d
        )
        # traj_all = [agent.trajectory for agent in aerial_team.agents]
        # traj_all = np.array(traj_all)
        # # Save trajectories
        # np.save(
        #     os.path.join(results_path, "aerial_trajectories.npy"),
        #     traj_all
        # )
        ...
        # )
        np.save(
            os.path.join(results_path, "final_aerial_density2.npy"),
            aerial_gp_mean.reshape(X_mpc.shape)
        )
