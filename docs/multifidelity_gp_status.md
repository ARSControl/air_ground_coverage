...
The uniform-sector-only sensor refinement is **complete and verified on
2026-07-16**. The LOW and HIGH simulated scalar-field sensors now
unconditionally draw area-uniform samples in each robot's configured FOV using
random bearing and `r = R*sqrt(U)`. The constructor/config selector and the
fixed-ray production branch were removed. GP mathematics, controller laws,
dynamics, update timing, and dedicated deterministic RNG streams are unchanged.
